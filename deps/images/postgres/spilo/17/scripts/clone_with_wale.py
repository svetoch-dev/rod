...
def get_clone_envdir():
    ...
    restore_command = shlex.split(
        config["bootstrap"]["clone_with_wale"]["recovery_conf"]["restore_command"]
    )
...
def get_wale_environments(env):
    ...
    names = [
        ...
        if name.endswith("_PREFIX") and name.startswith(prefix) and len(name) > 12
    ]
    if len(names) != 1:
        raise Exception(
            "Found find {0} {1}*_PREFIX environment variables, expected 1".format(
                len(names), prefix
            )
        )
    ...
    if "/spilo/" in value and value.endswith(
        "/wal"
    ):  # path crafted in the configure_spilo.py?
...
def run_clone_from_s3(options):
    ...
    if not options.dry_run:
        ...
        if (
            update_envdir
        ):  # We need to update file in the clone envdir or restore_command will fail!
