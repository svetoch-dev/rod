...
To run `plan/apply` in certain `root_module` you need to `cd terraform/environments/<env>/<root_module>/` and then execute `bazel run :plan` or `bazel run :apply`. Example `cd terraform/environments/gcp; bazel run plan`
